...
function parseBudget(text: string): string | null {
  ...
  const singleWord = text.match(
    /(?:budget|price|around|about|roughly|environ|autour\s+de|pr[eè]s\s+de|pres\s+de|up to|under|max(?:imum)?|jusqu[''][àa])?\s*:?\s*([\d\s.,]+)\s*(k|m|b|thousand|million|millions|milliard|milliards|mil|mille)\b(?:\s*(?:de\s+)?(?:mad|dirhams?|dh|usd|dollars?|eur|euros?|dollars?))?/i
  );
  if (singleWord) {
    ...
    if (n != null) {
      ...
      if (/\b(around|about|roughly|environ|autour)\b/i.test(text)) return `Around ${formatted}`;
      if (/\b(up to|under|max|jusqu)/i.test(text)) return `Up to ${formatted}`;
      return formatted;
    }
  }
...
function extractSignals(text: string): Partial<Collected> {
  ...
  const propRules: [RegExp, string][] = [
    ...
    [/\b(apartment|apt|flat|appartement)\b/, "Apartment"],
    [/\b(studio)\b/, "Studio"],
    [/\b(loft)\b/, "Loft"],
    [/\b(duplex)\b/, "Duplex"],
    [/\b(villa)\b/, "Villa"],
  ...
  const loc =
    text.match(
      /\b(?:in|near|around|looking in|prefer|area of|\u00e0|a|au|aux|en)\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)?(?:,\s*[A-Z]{2})?)/
    ) ||
    text.match(
  ...
  if (/\b(asap|immediately|right away|urgent|imm[eé]diatement|d[eè]s que possible|au plus vite|rapidement)\b/i.test(t)) {
    out.timeline = "Immediate / ASAP";
  } else if (/\b(this month|within (a |1 |one )?month|30 days|ce mois|dans (le )?mois)\b/i.test(t)) {
    out.timeline = "Within 30 days";
  } else if (/\b(1[-–—]3 months|next (few|couple of) months|60[-–—]90|dans quelques mois|prochainement)\b/i.test(t)) {
    out.timeline = "1–3 months";
  } else if (/\b(3[-–—]6 months|this (year|summer|fall)|cette ann[eé]e)\b/i.test(t)) {
    out.timeline = "3–6 months";
  } else if (/\b(6\+?\s*months|next year|no rush|flexible on timing|pas press[eé])\b/i.test(t)) {
    out.timeline = "6+ months / flexible";
  } else {
    const tm =
      t.match(/\b(?:within|in|about|around|d['']ici|dans|sous)\s+(\d+)\s*(days?|weeks?|months?|jours?|semaines?|mois)\b/i) ||
